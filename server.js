...
app.post('/api/register', (req, res) => {
  ...
  if (users[username]) {
    return res.status(409).json({ error: 'Username already exists. Try to login.' });
  }

  if (Object.values(users).find(u => u.email === email)) {
    return res.status(409).json({ error: 'Email already exists. Try to login.' });
  }

  if (Object.values(users).find(u => u.mpesa === mpesa)) {
    return res.status(409).json({ error: 'M-Pesa number already exists. Try to login.' });
  }
...
async function checkWithdrawDeposit(exchange, symbol) {
  ...
  if (
    ...
    Date.now() - statusCache[cacheKey].time < CACHE_TIME
  ) {
  ...
  try {
    if (exchange === 'kucoin') {
      const data = await safeGet(
        `https://api.kucoin.com/api/v1/currencies/${symbol}`,
        'KuCoin'
      );
      ...
      const coin = data?.data?.currencies?.find(
        c => c.currency === symbol
      );
      ...
      if (coin) {
        ...
        result.networks = [{
          name: 'MAIN',
          deposit: coin.deposit_enabled,
          withdraw: coin.withdraw_enabled
        }];
      }
    }
      ...
      const data = await safeGet(
        `https://api.gateio.ws/api/v4/wallet/currency_chains?currency=${symbol}`,
        'Gate.io'
      );
      ...
      const data = await safeGet(
        'https://api.coinex.com/v1/common/asset/config',
        'CoinEx'
      );
      ...
      const data = await safeGet(
        `https://www.okx.com/api/v5/asset/currencies?ccy=${symbol}`,
        'OKX'
      );
...
async function getMaxTradeable(exchange, symbol, side, price) {
  try {
    ...
    if (exchange === 'mexc') {
      url = `https://api.mexc.com/api/v3/depth?symbol=${symbol}USDT&limit=50`;
    }

    else if (exchange === 'kucoin') {
      url = `https://api.kucoin.com/api/v1/market/orderbook/level2_50?symbol=${symbol}-USDT`;
    }

    else if (exchange === 'bitget') {
      url = `https://api.bitget.com/api/spot/v1/market/depth?symbol=${symbol}USDT&limit=50`;
    }

    else if (exchange === 'gateio') {
      url = `https://api.gateio.ws/api/v4/spot/order_book?currency_pair=${symbol}_USDT&limit=50`;
    }

    else if (exchange === 'okx') {
      url = `https://www.okx.com/api/v5/market/books?instId=${symbol}-USDT&sz=50`;
    }

    else if (exchange === 'bybit') {
      url = `https://api.bybit.com/v5/market/orderbook?category=spot&symbol=${symbol}USDT&limit=50`;
    }

    else if (exchange === 'bitfinex') {
      url = `https://api-pub.bitfinex.com/v2/book/t${symbol}USD/P0?len=50`;
    }

    else if (exchange === 'poloniex') {
      url = `https://api.poloniex.com/markets/${symbol}_USDT/orderBook?limit=50`;
    }
    ...
    if (exchange === 'mexc') {
      orders = side === 'buy' ? data.asks : data.bids;
    }
    ...
    for (const order of orders) {
      ...
      const slippage =
        Math.abs(orderPrice - price) / price;
...
function extractSymbolAndData(symbol, exchange, tickerData) {
  ...
  if (exchange === 'huobi' && symbol.endsWith('usdt')) {
  ...
  if (
    ...
    sym = symbol.replace('t', '').replace('USD', '');
    price = parseFloat(tickerData[7]);
    volume = parseFloat(tickerData[8]) * price;
  }

  if (exchange === 'poloniex' && symbol.endsWith('_USDT')) {
    sym = symbol.replace('_USDT', '');
    price = parseFloat(tickerData.close);
    volume = parseFloat(tickerData.amount) * price;
  }

  if (exchange === 'cryptocom' && symbol.endsWith('_USDT')) {
    sym = symbol.replace('_USDT', '');
    price = parseFloat(tickerData.a);
    volume = parseFloat(tickerData.v) * price;
  }

  if (exchange === 'upbit' && symbol.endsWith('-USDT')) {
    sym = symbol.replace('-USDT', '');
  ...
  try {
    ...
    for (const symbol of allSymbols) {
      ...
      Object.keys(allData).forEach(ex => {
        if (allData[ex][symbol]?.price) {
          prices[ex] = allData[ex][symbol];
        }
      });
      ...
      const [sellEx, sellData] =
        sorted[sorted.length - 1];

      const profitPct =
        (
          (
            sellData.price * 0.999 -
            buyData.price * 1.001
          ) /
          (buyData.price * 1.001)
        ) * 100;
    ...
    for (const opp of topCandidates) {
      ...
      if (
        buyStatus.canWithdraw !== false &&
        sellStatus.canDeposit !== false
      ) {
        verified.push({
          ...opp,
          ...
          status_unknown:
            buyStatus.canWithdraw === null ||
            sellStatus.canDeposit === null
        });
      }
    }
